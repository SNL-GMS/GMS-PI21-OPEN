...
public abstract class Waveform extends Timeseries {
   ...
   */
  @JsonCreator
  public static Waveform create(@JsonProperty("startTime") Instant startTime,
    @JsonProperty("sampleRateHz") double sampleRateHz,
    @JsonProperty("samples") double[] values) {
    Objects.requireNonNull(startTime);
    Preconditions.checkArgument(sampleRateHz > 0.0,
    ...
    values = Arrays.stream(values)
      .map(val -> BigDecimal.valueOf(val).setScale(10, RoundingMode.HALF_UP).setScale(4, RoundingMode.HALF_UP).doubleValue())
      .toArray();
  ...
  public Waveform trim(Instant start, Instant end) {
    ...
    Preconditions.checkState(this.intersects(start, end),
  ...
  public Waveform window(Instant start, Instant end) {
    Objects.requireNonNull(start);
    Objects.requireNonNull(end);
    Preconditions.checkArgument(!start.isBefore(this.getStartTime()),
      "new start is before startTime of this waveform");
    Preconditions.checkArgument(!end.isAfter(this.getEndTime()),
    ...
    final double[] newValues = Arrays.copyOfRange(this.getSamples(),
