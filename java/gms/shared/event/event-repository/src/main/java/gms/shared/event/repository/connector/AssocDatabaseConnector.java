...
public class AssocDatabaseConnector {
  ...
  public AssocDatabaseConnector(EntityManager entityManager) {
  ...
  public List<AssocDao> findAssocsByArids(Collection<Long> arids) {
    ...
    if (arids.isEmpty()) {
      ...
      return Lists.partition(new ArrayList<>(arids), 1000).stream()
        .map(partitionedArids -> {
  ...
  public List<AssocDao> findAssocsByOrids(Collection<Long> orids) {
    ...
    if (orids.isEmpty()) {
      ...
      return Lists.partition(new ArrayList<>(orids), 1000).stream()
        .map(partitionedOrids -> {
          var criteriaBuilder = entityManager.getCriteriaBuilder();
          var query = criteriaBuilder.createQuery(AssocDao.class);
          var fromAssoc = query.from(AssocDao.class);
          var idPath = fromAssoc.get(ID);
  ...
  public List<AssocDao> findAssocsByAridsAndOrids(Collection<Pair<Long, Long>> aridOridList) {
    ...
    if (aridOridList.isEmpty()) {
      ...
      return Lists.partition(new ArrayList<>(aridOridList), 1000).stream()
        .map(partitionedAridsOrids -> {
