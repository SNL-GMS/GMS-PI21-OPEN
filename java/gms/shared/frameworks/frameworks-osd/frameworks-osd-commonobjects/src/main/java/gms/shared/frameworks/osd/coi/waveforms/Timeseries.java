...
@JsonTypeInfo(
  ...
  property = "type")
@JsonTypeIdResolver(TimeseriesTypeIdResolver.class)
public abstract class Timeseries implements Comparable<Timeseries> {
  ...
  public enum Type {
    ...
    FK_SPECTRA(FkSpectra.class),
    DETECTION_FEATURE_MAP(null);

    private static final ImmutableMap<String, Type> TYPES_BY_NAME;
    ...
    static {
      ...
      TYPES_BY_NAME = builder.build();
    }

    private final Class<? extends Timeseries> implementingClass;

    private Type(Class<? extends Timeseries> implementingClass) {
      this.implementingClass = implementingClass;
    }

    public Class<? extends Timeseries> getImplementingClass() {
  ...
  public Duration getSamplePeriod() {
    return Duration.ofNanos((long) (1.0e9 / getSampleRate()));
  }
  ...
  public Instant computeSampleTime(int i) {
    ...
    final double nanosPerSample = 1E9 / getSampleRate();
