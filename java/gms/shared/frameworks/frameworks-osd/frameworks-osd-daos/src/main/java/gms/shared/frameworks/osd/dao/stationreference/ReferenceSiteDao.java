...
public class ReferenceSiteDao {
  ...
  @SequenceGenerator(name = "reference_site_sequence", sequenceName = "reference_site_sequence", allocationSize = 5)
  private long id;
  ...
  @JoinTable(
    ...
    joinColumns = {@JoinColumn(name = "reference_site", table = "reference_site", referencedColumnName = "id")},
  ...
  public ReferenceSite toCoi() {
    return ReferenceSite.builder()
      ...
      .setAliases(getAliases().stream().map(ReferenceAliasDao::toCoi).collect(Collectors.toList()))
      .build();
  }
  ...
  public boolean equals(Object o) {
    ...
    if (!(o instanceof ReferenceSiteDao)) {
      return false;
    }
  ...
  @Override
  public int hashCode() {
    return Objects.hash(entityId, versionId, name, description, latitude, longitude, elevation,
      comment, source, position, actualTime, systemTime, active, aliases);
  }
