...
public abstract class DelegatingRequestHandler<T, U> implements RequestHandler<U> {
   ...
   * to an instance of T, or the handler operation throws a {@link InvalidInputException}.
   * The response is a {@link Response#serverError(String)} if the handler
  ...
  public Response<U> handle(Request request, ObjectMapper objectMapper) {
    ...
    try {
      ...
      return Response.clientError(logAndReturnErrorMsg(
        "handlerOperation rejected input", e));
    } catch (Exception e) {
