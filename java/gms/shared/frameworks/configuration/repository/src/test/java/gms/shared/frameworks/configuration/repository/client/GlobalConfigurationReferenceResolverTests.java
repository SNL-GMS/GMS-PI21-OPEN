...
class GlobalConfigurationReferenceResolverTests {
  ...
  private static Map<String, ConfigurationSelectorCache> configurationsByKey(int num) {
    ...
    return Map.of(CONFIG_KEY,
      ConfigurationSelectorGuavaCache.create(Configuration.from(CONFIG_KEY, options), Duration
        .ofDays(1)));
  }
  ...
  void testNonGlobalConfigReferenceThrowsException() {
    ...
    Map<String, Object> parameters = FieldMapUtilities
  ...
  void testFileFilterReferenceExample() throws JsonProcessingException {
    ...
    Map<String, Object> globalMap = u
      .resolve("global.filters", List.of(Selector.from("name", "filter4-8")));
    ...
    Assertions.assertEquals(1, componentMap.get("numeric"));
  ...
  void testSubstituteReferenceHasNullParameterValue() {
    Map<String, Object> result = GlobalConfigurationReferenceResolver
      .resolve(buildConfigurationWithNullParameterValue(), buildConfigRef(0));
   ...
   */
  private static Map<String, ConfigurationSelectorCache> buildConfigurationWithNullParameterValue() {
    String name = getConfigName(0);

    final ConfigurationOption option = ConfigurationOption.from(name,
      List.of(
        StringConstraint.from("name",
          Operator.from(Type.EQ, false),
          Set.of(name), 100)),
    ...
    return Map.of(CONFIG_KEY, ConfigurationSelectorGuavaCache
      .create(Configuration.from(CONFIG_KEY, List.of(option)), Duration.ofDays(1)));
  }
