...
class ConfigurationResolverTests {
  ...
  void setUp() {
    ...
    fooParamsDefaultsFieldMap = FieldMapUtilities.toFieldMap(fooParamsDefaults);
    ...
    final ConfigurationOption configOptSnrIs5Default = ConfigurationOption
      .from("GLOBAL-DEFAULT", List.of(DefaultConstraint.from(), snrIs5, bazIsWildcard),
        FieldMapUtilities.toFieldMap(fooParamsSnr5Defaults));
    ...
    snrIs5ParametersFieldMap = FieldMapUtilities.toFieldMap(
      fooParamsDefaults.toBuilder().a(10).build());
    snrIs5WIthSnr5DefaultParametersFieldMap = FieldMapUtilities.toFieldMap(
      fooParamsSnr5Defaults.toBuilder().a(10).build());
    fooParamsSnrIs5BazIs10FieldMap = FieldMapUtilities.toFieldMap(
      fooParamsDefaults.toBuilder().a(20).build());
    fooParamsSnrIs5BazIsWildcardFieldMap = FieldMapUtilities.toFieldMap(
  ...
  void testResolveUsesPriorityToBreakTies() {
    ...
    final Map<String, Object> fooParamsSnrIsWildcardBazIs10 = FieldMapUtilities
  ...
  @Test
  void testResolveWorksWhenNumericScalarSelectorIsInt() {
    final ConfigurationOption configOptSnrIsWildcardBazIs10 = ConfigurationOption
      .from("(SNR-WILDCARD)/(BAZ-10)", List.of(snrIsWildcard, bazIs10), Map.of("a", 40));
    ...
    assertEquals(fooParamsSnrIsWildcardBazIs10, ConfigurationResolver.resolve(configuration,
      List.of(Selector.from("baz", 10), Selector.from("snr", 5.0))));
  }
   ...
   */
  @Test
  void testResolveHasTieExpectIllegalStateException() {
