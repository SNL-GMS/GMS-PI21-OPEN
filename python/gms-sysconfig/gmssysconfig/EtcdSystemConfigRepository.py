...
class EtcdSystemConfigRepository:
    ...
    def __init__(self, endpoints: str | None = None, username: str | None = None, password: str | None = None, verbose: bool = False) -> None:
        # If no endpoints were specified, get them from our environment
        self.endpoints = endpoints if endpoints else os.getenv('GMS_ETCD_ENDPOINTS', 'etcd:2379')
        self.username = username if username else os.getenv('ETCD_GMS_USER')
        self.password = password if password else os.getenv('ETCD_GMS_PASSWORD')  # known password for read-only access
        self.verbose = verbose

        if not self.username:
            raise ValueError(f"No etcd user specified.")

        if not self.password:
            raise ValueError(f"No etcd password specified.")
