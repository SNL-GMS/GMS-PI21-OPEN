...
lm.utils.getUniqueId = function() {
  ...
  return (random() * 1000000000000000).toString(36).replace('.', '');
};
...
lm.utils.copy(lm.LayoutManager.prototype, {
  ...
  init: function() {
    ...
    if (electron) {
      electron.ipcRenderer.on('popin-window-resolve', (event, args) => {
        let config = args;
        let parentId = config.parentId;
        let indexInParent = config.indexInParent;
         ...
         */
        childConfig = config; //$.extend(true, {}, this.getGlInstance().toConfig()).content[0];
        parentItem = this.root.getItemsById(parentId)[0];
         ...
         */
        if (!parentItem) {
          if (this.root.contentItems.length > 0) {
            parentItem = this.root.contentItems[0];
          } else {
        ...
        parentItem.addChild(childConfig, indexInParent);
      });
    }
  },
  ...
  _adjustToWindowMode: function() {
    ...
    popInButton.click(
      lm.utils.fnBind(function() {
        let electron = require('electron');
        if (electron) {
          electron.ipcRenderer.send('popin-window', this.config);
...
lm.utils.copy(lm.controls.BrowserPopout.prototype, {
  ...
  _createWindow: function() {
    ...
    var checkReadyInterval,
       ...
       */
      title = Math.floor(random() * 1000000).toString(36),
      /**
    ...
    let electron = require('electron');
    if (electron) {
      let thisWindow = electron.remote.getCurrentWindow();
      electron.ipcRenderer.send('popout-window', {
        config: this._config,
        title: this._config[0].title,
        url: window.location.origin + '/#/' + this._config[0].component, //url,
        bounds: {
          x: thisWindow.getBounds().x,
          y: thisWindow.getBounds().y,
  ...
  _scheduleEventPropagationToLayoutManager: function(name, event) {
    if (lm.utils.indexOf(name, this._throttledEvents) === -1) {
      ...
      if (this._pendingEventPropagations[name] !== true) {
        ...
        lm.utils.animFrame(
          lm.utils.fnBind(this._propagateEventToLayoutManager, this, [name, event])
        );
      }
    }
  },
