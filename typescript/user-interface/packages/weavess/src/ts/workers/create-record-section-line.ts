...
function convertWaveformYToCanvasY(waveformYArray: number[], distanceDegrees: number) {
  ...
  const sortedYArray = waveformYArray.slice(0, waveformYArray.length).sort(numericSortAsc);
  ...
  const [correctedYMin, correctedYMedian, correctedYMax] = [yMin, yMedian, yMax].map(
    val => yOffset + ((val - yMedian) / yRange) * logicalWaveformHeight
  );
  ...
  const correctedY = waveformYArray.map(
    y => ((y - yMedian) / yRange) * logicalWaveformHeight + yOffset
  );
 ...
 * @param params
 */
export const createRecordSectionPositionBuffer = (
  params: CreateRecordSectionLineParams
): Float32Array => {
  const vertices: number[] = [];

  // Convert the raw waveform coordinates to record section coordinates.
  const waveformCanvasY = convertWaveformYToCanvasY(
    params.data,
    kilometersToDegrees(params.distance)
  );

  // Draw the waveform.
  for (let j = 0; j < waveformCanvasY.yArr.length; j += 1) {
    const t =
      params.defaultCameraLeft + (j * params.cameraXRange) / (waveformCanvasY.yArr.length - 1);
    vertices.push(t);
