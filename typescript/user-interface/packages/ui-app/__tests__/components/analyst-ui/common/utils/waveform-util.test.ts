...
import {
  ...
  getWaveformFilterForMode
} from '../../../../../src/ts/components/analyst-ui/common/utils/waveform-util';
...
describe('getWaveformFilterForMode', () => {
  ...
  test('check for getting the filter for DEFAULT mode with no filters', () => {
    expect(
      getWaveformFilterForMode(AnalystWorkspaceTypes.WaveformDisplayMode.DEFAULT, sampleRate20, [])
    ).toEqual(undefined);
    expect(
      getWaveformFilterForMode(AnalystWorkspaceTypes.WaveformDisplayMode.DEFAULT, sampleRate40, [])
    ).toEqual(undefined);
  });
  ...
  test('check for getting the filter for MEASUREMENT mode for sample rate of 20', () => {
    const filter = getWaveformFilterForMode(
      AnalystWorkspaceTypes.WaveformDisplayMode.MEASUREMENT,
      sampleRate20,
      defaultFilters
    );
    expect(filter.filterType).toEqual(systemConfig.measurementMode.amplitudeFilter.filterType);
    expect(filter.filterPassBandType).toEqual(
      systemConfig.measurementMode.amplitudeFilter.filterPassBandType
    );
    expect(filter.lowFrequencyHz).toEqual(
      systemConfig.measurementMode.amplitudeFilter.lowFrequencyHz
    );
    expect(filter.highFrequencyHz).toEqual(
      systemConfig.measurementMode.amplitudeFilter.highFrequencyHz
    );
...
describe('getSelectedWaveformFilter', () => {
  ...
  const channelFilters = { [id]: channelFilter };

  test('check for getting the filter for MEASURMENT mode for sample rate 20', () => {
    const filter = getSelectedWaveformFilter(
      AnalystWorkspaceTypes.WaveformDisplayMode.MEASUREMENT,
      undefined,
      sampleRate20,
      {},
  ...
  test('check for getting the filter for DEFAULT mode with no channel filters for sample rate 20', () => {
    const filter = getSelectedWaveformFilter(
      AnalystWorkspaceTypes.WaveformDisplayMode.DEFAULT,
      undefined,
      sampleRate20,
      {},
      defaultFilters
